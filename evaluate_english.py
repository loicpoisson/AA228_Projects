...
def render_console(env):
    ...
    scientific_value = env.payload * 50
...
def save_trajectory(initial_grid, path, title="Autonomous Rover Trajectory", filename="trajectory.png"):
    ...
    # --- MODIFICATION 3: Use the dynamic title ---
    ax.set_title(title)
...
def run_demo():
    ...
    # --- MODIFICATION 4: Final print and pass title to save_trajectory ---
    final_sci_value = env.payload * 50
    print(f"End of demo. Total Score: {total_reward} | Final Scientific Value: {final_sci_value}")

    # Optionally save trajectory visualization with dynamic title
    save_trajectory(
        initial_grid, 
        path, 
        title=f"Autonomous Rover Trajectory (Scientific Value: {final_sci_value})",
